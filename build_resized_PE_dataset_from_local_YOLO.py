    ...
    for idx, entry in tqdm(enumerate(dataset), desc=f"Processing {view_type} images", total=len(dataset)):
        ...
        for (bbox, class_id) in bboxes:
            ...
            if cropped_image is not None:
                length = max(cropped_image.shape[:2])
                canvas = paste_on_black_canvas(cropped_image, length, length)
                ...
                y_offset = (length - h) // 2 - y_min  # y_min is the original image y_min of the bbox
                x_offset = (length - w) // 2 - x_min  # x_min is the original image x_min of the bbox
                ...
                if sum(map(lambda x: (x<0) or (x>length), adjusted_keypoint["x"])) >= 1:
                    print(f"Some keypoints X out of bound: {cropped_filename}")
                if sum(map(lambda y: (y<0) or (y>length), adjusted_keypoint["y"])) >= 1:
