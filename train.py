...
model = UNet(n_channels=1, n_classes=18).to(device)
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(train_loader, desc=f"Epoch: {epoch+1}/{num_epochs} [TRAIN]"):
        ...
        keypoints_heatmaps = generate_heatmaps(keypoints, heatmap_size=outputs.shape[2:], sigma=5)
        ...
        for idx, batch in tqdm(enumerate(val_loader), desc=f"Epoch: {epoch+1}/{num_epochs} [VALID]", total=len(val_loader)):
            ...
            if idx == 0:
                for i in range(images.size(0)):
                    ...
                    image = images[i]
                    heatmaps = outputs[i]
                    save_path_heatmap = f"results/heatmap_overlay_pred_{idx}_{i}.png"
                    visualize_heatmaps(image, heatmaps, save_path=save_path_heatmap)
                    heatmaps = keypoints_heatmaps[i]
                    save_path_heatmap = f"results/heatmap_overlay_truth_{idx}_{i}.png"
                    visualize_heatmaps(image, heatmaps, save_path=save_path_heatmap)
