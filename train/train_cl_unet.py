...
from train.utils import InfoNCELoss, get_lr, generate_heatmaps, visualize_heatmaps, visualize_peaks_from_heatmap
...
for epoch in range(config.num_epochs):
    ...
    for batch in tqdm(train_loader, desc=f"Epoch: {epoch+1}/{config.num_epochs} [TRAIN]"):
        ...
        # reps_front = [ torch.nn.functional.avg_pool3d(reps["front"], 2) ]
        # reps_top   = [ torch.nn.functional.avg_pool3d(reps["top"],   2) ]
            ...
            if idx == 0:
                for view in [ "front", "top" ]:
                    for i in range(images[view].size(0)):
                        if i == 8: break
                        image = images[view][i]
                        visualize_heatmaps(image, htmp_pred[view][i, :1], save_path=f"results/cl_unet_tds={training_dataset_size}/heatmap_{i}_{view}_pred.png")
                        visualize_heatmaps(image, htmp_gt[view][i, :1], save_path=f"results/cl_unet_tds={training_dataset_size}/heatmap_{i}_{view}_truth.png")
                        visualize_peaks_from_heatmap(image, htmp_pred[view][i], save_path=f"results/cl_unet_tds={training_dataset_size}/peaks_{i}_{view}_pred.png", scale=scale)
                        visualize_peaks_from_heatmap(image, htmp_gt[view][i], save_path=f"results/cl_unet_tds={training_dataset_size}/peaks_{i}_{view}_truth.png", scale=scale)
